...
+ Faster interpreter startup time; 18ms to 2ms in a test machine +
+ Loading plain Lua source is now done through `package.loaders` +
+ Easier standalone programs +
+ Now a package to support ./cmd builds +
+ Added crypto.random +
+ Added crypto.fast_random +
+ Added kapow.redirect_permanent +
+ Added kapow.redirect_temporary +
+ Added kapow.redirect_post +
+ New module: guard +
+ New module: deque +
+ New module: bimap +
+ New module: tuple +
+ New module: argparse +
+ Added back inspect.lua +
+ Added table.clone +
...
. Programs go in `./cmd`
. Use `./cmd/standalone` as a basis
. In the example program, the *main* Lua source goes into `src/main.lua`
. See `./scripts/build-standalone` for command to build your program
